...
import type { StorefrontSettings, GitHubTestResult, ImageUrlTestResult } from "./types";
...
export default function SettingsTab({ settings, onSaved, onToast }: Props) {
  const [form, setForm] = useState({
    ...
    github_token:          "",   // always blank on load — existing token is preserved server-side
    github_repo:           settings?.github_repo           ?? "",
    github_file_path:      settings?.github_file_path      ?? "public/collectibles/catalog.json",
    github_commit_message: settings?.github_commit_message ?? "chore: publish catalog ({n} items)",
  });
  ...
  const handleTestGithub = async () => {
    ...
    try {
      const result = await api<GitHubTestResult>("/storefront/settings/test-github", {
        method: "POST",
    ...
    try {
      const saved = await api<StorefrontSettings>("/storefront/settings", {
        ...
        body: JSON.stringify({
          ...
          github_token:          form.github_token          || null,  // null = keep existing
          github_repo:           form.github_repo           || null,
          github_file_path:      form.github_file_path      || null,
          github_commit_message: form.github_commit_message || null,
        }),
      });
      onSaved(saved);
  ...
  const hasGitHub = settings?.github_token_set && form.github_repo;
      ...
      >
        ...
        >
          <div className="relative">
            ...
            <input
              ...
              placeholder={settings?.github_token_set
                ? `Current token: ${settings.github_token_hint} — leave blank to keep`
                : "github_pat_…"}
          ...
          {settings?.github_token_set && (
            <div className="flex items-center gap-1.5 text-xs text-emerald-700 mt-1">
              ...
              Token configured ({settings.github_token_hint}). Leave blank to keep it.
            </div>
          )}
        </Field>
        ...
        <div>
          <button
            ...
            disabled={testingGithub || !form.github_repo || (!form.github_token && !settings?.github_token_set)}
            className="flex items-center gap-1.5 px-3 py-1.5 border border-slate-200 rounded-lg text-xs font-medium text-slate-600 hover:bg-slate-50 disabled:opacity-50"
          >
        ...
        >
          <input
            ...
            onChange={e => set("github_file_path", e.target.value)}
        ...
        >
          <input
            value={form.github_commit_message}
            onChange={e => set("github_commit_message", e.target.value)}
