    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-50 via-sky-50 to-emerald-50 text-slate-800">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 grid gap-6">
        ...
        {isModuleEnabled("real_estate") && activeModule === "real_estate" && (
          currentUser ? (
            <RealEstateModule userId={userId} onToast={addToast} onNavigate={handleSelectModule} />
          ) : (
            <GuestPrompt onSignUp={() => openAuth('signup')} onSignIn={() => openAuth('login')} />
          )
        )}
        ...
        {/* ── AI Assistant module ── */}
        {isModuleEnabled("ai_assistant") && activeModule === "ai_assistant" && (
          currentUser ? (
            <AiAssistantModule userId={userId} onToast={addToast} />
          ) : (
            <GuestPrompt onSignUp={() => openAuth('signup')} onSignIn={() => openAuth('login')} />
          )
        )}
