...
class ApiToken(SQLModel, table=True):
    """
    Personal access token for external agents (e.g. Claude Code via MCP) to
    call into a user's opama data. Format: `opat_<32 url-safe chars>`.

    token_hash is an HMAC-SHA256 hex digest (app.secrets.token_digest) — the
    raw token is shown to the user once at creation and never stored.
    A PAT grants the bearer the SAME permissions as the user, including
    mutating tools, with no per-call confirmation — see
    services/ai_assistant/router.py's MCP endpoint docstring.
    """
    ...
    hint: str                              # e.g. "opat_…a4f2"
    scopes: Optional[str] = None           # reserved, unused v1
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    last_used_at: Optional[datetime] = None
