...
class LLMMessage:
    ...
    content: str
    tool_calls: Optional[List[ToolCall]] = None  # assistant turn that requested tools
    tool_call_id: Optional[str] = None  # "tool" role: which call this answers
    name: Optional[str] = None  # "tool" role: tool name
...
class LLMProvider(ABC):
    ...
    def chat(
        ...
        *,
        ...
        """Plain chat completion. Pass `tools` to enable tool-calling — if
        the model chooses to call one, the result's `tool_calls` is
        populated and `content` may be empty."""
