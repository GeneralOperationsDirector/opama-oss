...
These tests run on the host with only PyYAML and packaging (no fastapi needed).
...
# ---------------------------------------------------------------------------
# Core compatibility (requires_core — see docs/RELEASE_PROCESS.md)
#
# requires_core is optional; none of opama's in-repo manifests declare it
# today, so this class is currently a no-op. It exists to enforce the
# contract for future manifests (including ones generated by
# scripts/new_module.py): if a manifest opts in to requires_core, the
# specifier must be valid, and the core version it ships with must satisfy
# its own declared range.
# ---------------------------------------------------------------------------

class TestCoreCompatibility:

    def test_requires_core_is_valid_specifier(self, all_manifests):
        """If declared, requires_core must be a valid PEP 440 specifier set."""
        for m in all_manifests:
            requires_core = m.get("requires_core")
            if not requires_core:
                continue
            try:
                SpecifierSet(requires_core)
            except InvalidSpecifier as exc:
                pytest.fail(f"Invalid requires_core '{requires_core}' in {m['_path']}: {exc}")

    def test_core_version_satisfies_declared_requirements(self, all_manifests):
        """The shipped CORE_VERSION must satisfy every manifest's requires_core,
        if declared — a plugin shouldn't ship declaring itself incompatible
        with the core it ships with."""
        for m in all_manifests:
            requires_core = m.get("requires_core")
            if not requires_core:
                continue
            assert Version(CORE_VERSION) in SpecifierSet(requires_core), (
                f"CORE_VERSION {CORE_VERSION} does not satisfy requires_core "
                f"'{requires_core}' in {m['_path']}"
            )
